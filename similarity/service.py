...
import similarity.scoring as similarity_scoring
...
def _score_returned_metadata(app_profile, metadata_profile, *, title: str, abstract: str, raw) -> dict:
    """Score via the optional profile scorer without making service import fragile.

    Some downloaded/stale checkouts have had ``similarity.service`` imported alongside an
    older ``similarity.scoring`` module that did not expose ``score_profiles``.  Importing
    the scoring module (rather than the symbol) keeps app startup working; if the profile
    scorer is missing, records are deliberately left unscored instead of falling back to
    query-term matching and recreating false positives.
    """
    scorer = getattr(similarity_scoring, "score_profiles", None)
    if callable(scorer):
        return scorer(app_profile, metadata_profile, title=title, abstract=abstract, raw=raw)
    return {
        "score": 0.0,
        "risk": "NONE",
        ...
        "matched_dimensions": {},
        "why_relevant": "Similarity scoring profile support is unavailable in this checkout, so returned records were not scored from query terms alone.",
    }
...
def run_similarity_service(facts: ApplicationFacts, settings: Settings, run_similarity_check: bool = False, mock_mode: bool = False, snippets: list[str] | None = None) -> dict:
    ...
    for func, source in [(search_lens, "Lens Scholarly"), (search_epo, "EPO OPS"), (search_nihr_open_data, "NIHR Open Data")]:
        ...
        scored = _score_returned_metadata(app_profile, metadata_profile, title=title, abstract=abstract, raw=raw)
