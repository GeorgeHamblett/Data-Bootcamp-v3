...
def evaluate_requirement(req: Requirement, facts: ApplicationFacts) -> ChecklistItem:
    ...
    if missing_terms:
        return ChecklistItem(req.checklist_area, req.requirement_text, SOURCE_LABELS.get(req.source, req.source), "Partially present", "AMBER", evidence, f"Relevant evidence is present but missing or unclear: {', '.join(missing_terms)}.", PRACTICAL_ACTIONS.get(req.checklist_area, f"Add or verify {', '.join(missing_terms)}."), 0.65)
